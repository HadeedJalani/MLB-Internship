🏗️ Construction Equipment Detection Custom YOLO11 Object Detection
<p align="center"> <strong>A custom-trained YOLO11 model for detecting construction equipment in images and videos.</strong> </p> <p align="center"> Built using <strong>Ultralytics YOLO11</strong>, trained on a large-scale construction equipment dataset using Google Colab GPU, and deployed through an interactive Streamlit application. </p> <p align="center">
...
Detects construction equipment using a custom-trained YOLO11 model.

🖼️ Image Detection

Users can upload images and receive annotated predictions with:
...
Supports object detection on uploaded videos with frame-by-frame inference.
...
Users can control the minimum confidence required for predictions.

💾 Download Results

Annotated images and processed videos can be downloaded directly from the application.

☁️ Streamlit Deployment

The trained custom model is integrated into a deployment-ready Streamlit application.
...
Image Segmentation
      ↓
Exact pixels belonging to each object
⚡ Why YOLO?
...
The model was initialized using pre-trained weights:

model = YOLO("yolo11s.pt")

YOLO11s provides a strong balance between:
...
Source: Roboflow Universe — Equipment Dataset by SODA1
...
Users can upload supported video files for frame-by-frame object detection.
...
Collecting more tower crane examples
Targeted oversampling
Class-aware augmentation
Additional training iterations
2️⃣ Dump Truck Localization

The dump_truck class showed a noticeable difference between:

mAP@50      → 90.7%
mAP@50-95   → 68.6%

This suggests that the model successfully detects dump trucks but has more difficulty producing highly precise bounding box localization across stricter IoU thresholds.

Possible Cause

Dump trucks appear in the dataset with:
...
The training notebook was improved to:
...
Replace YOUR_STREAMLIT_APP_URL with your deployed Streamlit link.

📎 Submission Links
Resource	Link
💻 GitHub Repository	View Repository
🌐 Streamlit Application	Open App
🎥 Demo Video	Watch Demo
🎬 Model Output Video	Watch Detection Output
🛠️ Technology Stack
Technology	Purpose
...
<p align="center"> <strong>Built as part of the MLBench Summer Internship — Custom Object Detection Journey 🚀</strong> </p> <p align="center"> ⭐ If you found this project useful, consider starring the repository. </p>